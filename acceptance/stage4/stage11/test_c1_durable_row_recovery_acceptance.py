"""Recovery acceptance for C1 authority durable before its controller checkpoint."""
...
def test_restart_recovers_c1_authority_row_without_reinvoking_external_actors(
    tmp_path: Path,
) -> None:
    world = run_world(
        tmp_path,
        ...
        run_id="c1-durable-row-recovery",
    )
    ...
    authority_log = world.boundary.writer.path
    lf_records = authority_log.read_bytes().replace(b"\r\n", b"\n")
    authority_log.write_bytes(lf_records.replace(b"\n", b"\r\n"))
