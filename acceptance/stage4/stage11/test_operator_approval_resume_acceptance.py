...
@pytest.mark.parametrize("task_schema", [TASK_CONTRACT_SCHEMA_V1, TASK_CONTRACT_SCHEMA_V2], ids=["fixed-v1", "selected-v2"])
def test_one_operator_command_resumes_without_repreparing_or_reapproving(tmp_path, capsys, task_schema):
    world = run_world(
    ...
    if task_schema == TASK_CONTRACT_SCHEMA_V2:
        profile = replace(profile, task_contract=replace(profile.task_contract, schema_version=task_schema, behavior_refs=()))
