...
def test_fresh_composition_recovers_terminal_decision_without_external_calls(
    tmp_path: Path,
) -> None:
    world = run_world(
        tmp_path,
    ...
    second = create_composition(world).execute()
    ...
    assert world.worker_process.calls == before_worker
    assert world.oracle.calls == before_oracle
