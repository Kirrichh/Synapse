...
Состояние целевого Gold: местное применение памяти с запретом её отправки
внешней модели пока не реализовано. Сейчас содержимое допущенного знания
попадает в текстовую историю Mini и далее в запрос его модели. Поэтому зелёные
проверки стенда и приведённые ниже исторические прогоны не подтверждают
согласованный контур Gold. Приватное знание через этот путь передавать нельзя.
...
Mini, запрещённый зафиксированным профилем. Baseline решил задачу со второй
