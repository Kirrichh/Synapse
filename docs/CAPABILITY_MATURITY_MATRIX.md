...
| Distributed consensus | **Partial — P3a semantic core verified** | `P3a IMPLEMENTED / VERIFIED_ON_MAIN / S1/S2 EVIDENCE CLOSED` | P3a заменяет facade auto-commit на deterministic content-sensitive semantic consensus core: configured `VoteSource`, participant vote states, strict validation, canonical `proposal_id` / `votes_hash` / `result_hash`, and `distributed_consensus_decided` event. Production distributed consensus protocol behavior, network/daemon voting, actor-mailbox voting, live LLM voting and durable replay closure remain out of scope. Evidence: `docs/evidence/P3A_EVIDENCE.md`, PR #27 merge commit `60db4d3aa610c0cab6ec19cf532b47b7107de136`. |
