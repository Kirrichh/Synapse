...
Проверено относительно `main` на post-merge P3a commit `60db4d3aa610c0cab6ec19cf532b47b7107de136`.
...
| Distributed consensus | **Partial — P3b local actor-method vote source verified; P3c-0 replay consumption closed** | `P3a IMPLEMENTED / VERIFIED_ON_MAIN / S1/S2 EVIDENCE CLOSED`; `P3b POST_MERGE_ACCEPTED / EVIDENCE CLOSED`; `P3c-0 POST_MERGE_ACCEPTED / EVIDENCE CLOSED` | P3a provides the deterministic content-sensitive semantic consensus core: configured `VoteSource`, participant vote states, strict validation, canonical `proposal_id` / `votes_hash` / `result_hash`, and `distributed_consensus_decided` event. P3b verifies an explicit opt-in, synchronous local runtime actor-method vote source behind `VoteSource`; default behavior remains non-actor and explicit `VoteSource` overrides take precedence. P3c-0 closes canonical local replay consumption for `distributed_consensus_decided` only via `consensus.event.v2` with a normalized votes map; REPLAY consumes recorded votes through an engine-owned deterministic reducer path and fail-closed verifies `proposal_id`, `statement_identity`, `votes_hash`, and `result_hash` before the replay frontier. Broader durable consensus lifecycle closure for mailbox, promise, signal, and ticket stages remains future work. Production distributed consensus protocol behavior, mailbox voting, daemon voting, network voting, live LLM voting, and durable replay closure remain out of scope. Evidence: `docs/evidence/P3A_EVIDENCE.md`, `docs/evidence/P3B_EVIDENCE.md`, `docs/evidence/P3C_EVIDENCE.md`, PR #27 merge commit `60db4d3aa610c0cab6ec19cf532b47b7107de136`, PR #31 merge commit `dbdfc7252c83d9fc4be0f0b5eb2cbd2007f0e2ad`, PR #34 merge commit `16fdd5fb209a9ab387359888bf1952571cfe8fba`. |
