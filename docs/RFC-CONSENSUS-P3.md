...
For the purposes of this RFC, **content-sensitive consensus** means:
...
## 2. Current State and Facade Findings

### 2.1 Existing capability shape
...
### 4.1 P3a — Semantic Consensus Core
...
### 4.2 P3b — Runtime / Actor Vote Integration
...
### 4.3 P3c — Durable / Replay Closure
...
### 5.1 Existing grammar retained in P3a
...
### 5.2 No parser or AST expansion in P3a
...
### 5.3 `policy` clause interpretation in P3a
...
The P3a default quorum formula is an intentional semantic correction from facade-era behavior.

Current facade-era behavior computes default quorum as:

```text
max(1, (participant_count + 1) // 2 + 1)
```

The P3a default quorum formula for MajorityVote is:

```text
participant_count // 2 + 1
```

The facade-era formula over-counts the absolute-majority threshold by exactly 1 for every odd participant_count. This is a systematic correction, not an isolated participant_count = 3 case.

Before/after evidence MUST cover at least participant_count ∈ {1, 3, 5}:

```text
participant_count = 1: facade default quorum = 2, P3a default quorum = 1
participant_count = 3: facade default quorum = 3, P3a default quorum = 2
participant_count = 5: facade default quorum = 4, P3a default quorum = 3
```

Even participant counts are not expected to change under this formula comparison:

```text
participant_count = 2: facade default quorum = 2, P3a default quorum = 2
participant_count = 4: facade default quorum = 3, P3a default quorum = 3
participant_count = 6: facade default quorum = 4, P3a default quorum = 4
```

This correction is not a compatibility preservation target because the previous behavior belonged to the semantic facade and depended on an invalid consensus model with implicit vote injection.
...
## 6. Participant Identity Contract

### 6.1 Accepted participant identity sources
...
### 6.2 Canonical participant identifier
...
### 7.6 Explicit missing representation
...
### 7.7 Vote payload source values
...
### 7.8 Conflicting vote entries inside one P3a evaluation
...
### 8.2 Forbidden P3a VoteSource behavior
...
### 8.3 Conceptual VoteSource interface
...
### 8.3A Canonical P3a VoteSource Injection Seam

P3a MUST define exactly one canonical interpreter-to-VoteSource injection seam before implementation.

The seam is part of the P3a RFC contract. It is not a test-only helper, not a private test fixture, not daemon state, not actor mailbox state, not user-facing `.syn` vote syntax, not live LLM output, not network state, and not hidden interpreter fallback behavior.

The approved P3a seam is:

```text
an interpreter-owned configured VoteSource value available before `DistributedConsensusStmt` evaluation and read synchronously by the interpreter adapter during that evaluation
```

This section does not prescribe the final Python method name or class name. The implementation may choose names that fit the existing runtime structure, but the semantic seam is mandatory:

```text
configured VoteSource -> interpreter adapter -> consensus engine
```

The interpreter adapter MUST obtain P3a votes only through this configured VoteSource seam.

If no configured VoteSource is present, P3a MUST use a null deterministic VoteSource whose output is `missing` for every normalized participant.

The null deterministic VoteSource MUST NOT synthesize `yes`, `no`, or `abstain`.

The null deterministic VoteSource MUST NOT commit by default.

An explicit vote-map VoteSource may be used for embedding/runtime invocation and tests, provided it enters through the same configured VoteSource seam.

P3a tests MUST use the same configured VoteSource seam as non-test embedding/runtime invocation.

P3a MUST NOT implement a runtime VoteSource registry.

P3a MUST NOT discover VoteSource providers dynamically.

P3a MUST NOT allow implementation work to invent an alternate VoteSource path outside this RFC contract.

The configured VoteSource seam closes `SG-P3-001 — VOTE_SOURCE_UNDEFINED` for P3a only. Runtime VoteSource registry, actor-backed VoteSource, mailbox-backed VoteSource, daemon-backed VoteSource, network-backed VoteSource, and LLM-backed VoteSource remain deferred to later stages.

### 8.4 Future VoteSource registry
...
- MUST be recorded only as advisory metadata in P3a;
- does not automatically vote;
- does not expand participant set;
- does not override strategy;
- does not create quorum.

In P3a, coordinator is not part of semantic proposal identity.

Coordinator MUST NOT affect quorum calculation, participant normalization, vote normalization, vote counting, strategy evaluation, outcome selection, `proposal_id`, `votes_hash`, or `result_hash`.

Coordinator may appear in result and event payloads only as advisory metadata for observability.

### 9.4 Proposal identity
...
`coordinator` MUST NOT participate in P3a `proposal_id`.

Reason: `proposal_id` identifies semantic proposal content. The same semantic proposal MUST NOT receive a different `proposal_id` solely because it was initiated from a different advisory coordinator context.

### 9.5 Proposal identity and repeated execution
...
### 9.7 Canonical hashing and serialization
...
### 9.9 Early termination and early rejection
...
## 10. Policy and Strategy Semantics

### 10.1 `policy_ref` as strategy selector in P3a
...
### 10.2 No governance guard execution in P3a
...
## 11. Outcome and Error Model

### 11.1 Valid operational outcomes
...
### 11.2 `insufficient_quorum` in P3a
...
### 11.6 Controlled validation error
...
### 11.7 Closed semantic `reason` values in P3a

In P3a, `reason` is a semantic field.

Therefore, valid operational consensus results MUST use a closed P3a semantic `reason` value.

Approved P3a semantic `reason` values are:
...
P3a MUST NOT emit arbitrary free-form semantic `reason` values.

P3a MUST NOT use human-readable display text as the semantic `reason`.

Human-readable reason strings may be emitted only as advisory metadata and MUST NOT participate in semantic identity.

Validation failures are not operational consensus results and MUST NOT use these semantic `reason` values as a substitute for controlled validation errors.

For P3a `deferred` results, the semantic `reason` MUST be:

```text
pending_missing_votes
```

A P3a `deferred` result means terminal-in-current-synchronous-evaluation. It does not mean an active wait, scheduler registration, durable suspension, actor mailbox subscription, daemon subscription, or stateful ticket lifecycle.

## 12. Canonical Result Shape
...
The `reason` field in the result shape MUST use one of the closed semantic P3a reason values from §11.7.

Human-readable reason strings, if present in a future shape, MUST be separate advisory metadata and MUST NOT replace the semantic `reason`.

---

## 13. Canonical Event Schema
...
The `reason` field in `distributed_consensus_decided` MUST use one of the closed semantic P3a reason values from §11.7.

Human-readable reason strings, if present in a future event shape, MUST be separate advisory metadata and MUST NOT replace the semantic `reason`.

### 13.3 Event payload size discipline
...
### 14.2 Proposal ID as correlation
...
### 18.2 Consensus engine responsibilities
...
### 18.3 Interpreter adapter responsibilities
...
## 18A. Interpreter Impact / Interpreter Contract

### 18A.1 Interpreter role

The interpreter is an adapter, not the consensus engine.

`evaluate_distributed_consensus()` SHOULD become a narrow adapter:
...
### 19.3 Scope-control acceptance

| **ID** | **Case** | **Expected** |
| --- | --- | --- |
| AC-P3A-023 | proposal\_id | deterministic canonical hash, no UUID |
...
| AC-P3A-033 | invalid\_request structural failure | no binding, no consensus event |
...
| AC-P3A-045 | statement\_identity extraction | stable AST/source identity passed to engine |
...
Implementation remains blocked until every stop-gate has an explicit RFC status.

The `_UNDEFINED` suffix is retained as the stable stop-gate identifier from the audit phase. The suffix does not mean the gate remains open after this table assigns a resolved or deferred status.

| Stop-gate | RFC status | Resolution / deferral target |
|---|---|---|
| SG-P3-001 — VOTE_SOURCE_UNDEFINED | RESOLVED_IN_THIS_RFC_AFTER_8_3A | Resolved by `8.3A Canonical P3a VoteSource Injection Seam`. Runtime VoteSource registry remains deferred. |
| SG-P3-002 — PARTICIPANT_IDENTITY_UNDEFINED | RESOLVED_IN_THIS_RFC | Resolved by §6.1 through §6.4. |
| SG-P3-003 — DUPLICATE_PARTICIPANT_POLICY_UNDEFINED | RESOLVED_IN_THIS_RFC | Resolved by §6.4 and §6.5. |
| SG-P3-004 — QUORUM_FORMULA_UNDEFINED | RESOLVED_IN_THIS_RFC_AFTER_B4 | Resolved by §5.6 and the default-quorum correction evidence added by this addendum. |
| SG-P3-005 — OUTCOME_MODEL_UNDEFINED | RESOLVED_IN_THIS_RFC_AFTER_B5 | Resolved by §11 and the closed semantic reason values added by this addendum. |
| SG-P3-006 — POLICY_SEMANTICS_UNDEFINED | RESOLVED_IN_THIS_RFC | Resolved by §5.3 and §10.1. |
| SG-P3-007 — EVENT_SCHEMA_UNDEFINED | RESOLVED_IN_THIS_RFC | Resolved by §13. |
| SG-P3-008 — REPLAY_SEMANTICS_UNDEFINED | DEFERRED_TO_P3C | P3a durable/replay closure remains out of scope. P3c must define replay consumption and fail-closed matching. |
| SG-P3-009 — TICKET_SEMANTICS_UNDEFINED | RESOLVED_FOR_P3A_AND_DEFERRED_FOR_FUTURE_SCOPE | P3a no-ticket rule is resolved by §14.1 and §14.2. Stateful ticket lifecycle is deferred to P3b/P3c by §14.3 and §24. |
| SG-P3-010 — LIVE_LLM_EXCLUSION_UNDEFINED | RESOLVED_IN_THIS_RFC | Resolved by §4.4, §8.2, §17, and §25. |
| SG-P3-011 — FAILURE_MODEL_UNDEFINED | RESOLVED_IN_THIS_RFC | Resolved by §9.1. |
| SG-P3-012 — P3A_EXECUTION_MODEL_UNDEFINED | RESOLVED_IN_THIS_RFC | Resolved by §4.1 and §9.2. |
| SG-P3-013 — COORDINATOR_MODEL_UNDEFINED | RESOLVED_IN_THIS_RFC_AFTER_B3 | Resolved by the coordinator advisory classification added by this addendum. |
| SG-P3-014 — PROPOSAL_IDENTITY_UNDEFINED | RESOLVED_IN_THIS_RFC_AFTER_B3 | Resolved by §9.4, §9.5, §9.6, and the removal of coordinator from P3a `proposal_id`. |
| SG-P3-015 — VOTE_PERSISTENCE_UNDEFINED | RESOLVED_IN_THIS_RFC | Resolved by §9.8. |
| SG-P3-016 — EARLY_TERMINATION_UNDEFINED | RESOLVED_IN_THIS_RFC | Resolved by §9.9 and §10.3 through §10.5. |
| SG-P3-017 — POLICY_STRATEGY_UNDEFINED | RESOLVED_IN_THIS_RFC | Resolved by §5.3 and §10.1. |
| SG-P3-018 — INTERPRETER_BOUNDARY_UNDEFINED | RESOLVED_IN_THIS_RFC | Resolved by §18.1 and §18.3. |
| SG-P3-019 — CONSENSUS_ENGINE_BOUNDARY_UNDEFINED | RESOLVED_IN_THIS_RFC | Resolved by §18.1 and §18.2. |
| SG-P3-020 — EVENT_APPEND_OWNERSHIP_UNDEFINED | RESOLVED_IN_THIS_RFC | Resolved by §13.5 and §18.3. |
| SG-P3-021 — P3A_RUNTIME_MODE_BEHAVIOR_UNDEFINED | RESOLVED_IN_THIS_RFC | Resolved by §4.1, §5.7, §9.2, §11.3, and §16.1. |
| SG-P3-022 — P3A_GRAMMAR_BOUNDARY_UNDEFINED | RESOLVED_IN_THIS_RFC | Resolved by §5.1 and §5.2. |
| SG-P3-023 — VALIDATION_ERROR_BEHAVIOR_UNDEFINED | RESOLVED_IN_THIS_RFC | Resolved by §11.4 through §11.6. |
| SG-P3-024 — P3A_TICKET_POLICY_UNDEFINED | RESOLVED_IN_THIS_RFC | Resolved by §14.1, §14.2, and §14.4. |
| SG-P3-025 — SEMANTIC_VS_ADVISORY_FIELDS_UNDEFINED | RESOLVED_IN_THIS_RFC_AFTER_B3_AND_B5 | Resolved by §15 after coordinator and semantic reason clarifications are applied. |
| SG-P3-026 — GOVERNANCE_POLICY_REF_COLLISION_UNDEFINED | RESOLVED_IN_THIS_RFC | Resolved by §5.3, §10.1, and AC-P3A-044. |
| SG-P3-027 — STATEMENT_IDENTITY_EXTRACTION_UNDEFINED | RESOLVED_IN_THIS_RFC | Resolved by §9.6 and AC-P3A-045. |
| SG-P3-028 — CANONICAL_JSON_HISTORY_INTEGRITY_UNDEFINED | RESOLVED_IN_THIS_RFC | Resolved by §9.7, §13.2, and AC-P3A-047. |
| SG-P3-029 — INSUFFICIENT_QUORUM_REPRESENTATION_UNDEFINED | RESOLVED_IN_THIS_RFC | Resolved by §9.9, §11.2, and AC-P3A-048. |

P3a implementation remains blocked until this table is present in the RFC and every `AFTER_*` entry has the referenced clarification applied.

P3b, P3c, and P3d implementation remain blocked by their own future RFCs and must not be inferred from this P3a contract.

## 21. Required Test Groups
...
47. Default quorum semantic correction tests MUST cover at least participant_count ∈ {1, 3, 5}.
48. Default quorum semantic correction tests MUST prove participant_count = 1: facade default quorum = 2, P3a default quorum = 1.
49. Default quorum semantic correction tests MUST prove participant_count = 3: facade default quorum = 3, P3a default quorum = 2.
50. Default quorum semantic correction tests MUST prove participant_count = 5: facade default quorum = 4, P3a default quorum = 3.
51. Default quorum semantic correction tests MUST prove the correction applies systematically to odd participant counts.
52. Default quorum semantic correction tests MUST prove representative even participant counts, such as participant_count ∈ {2, 4, 6}, do not change under the formula comparison.

53. Every emitted P3a operational result uses only approved closed semantic reason values.
...
25. Evidence MUST show the before/after threshold for participant_count ∈ {1, 3, 5}.
26. Evidence SHOULD also show representative even participant counts, such as participant_count ∈ {2, 4, 6}, to prove that the correction specifically affects odd participant counts under the formula comparison.
27. The evidence report MUST state that this is an intentional semantic correction from facade-era behavior and not a compatibility preservation target.

28. Closed semantic reason value proof for committed, rejected, deferred, insufficient_quorum, explicit no, unanimous no, unanimous abstain, and pending missing votes.
...
## 24. Deferred Technical Debt / Future Patch Items

The following items are intentionally not part of P3a.

### P3-TD-001 — Strategy Grammar Cleanup

Evaluate adding `strategy <strategy_name>` as an alias or replacement for `policy <policy_ref>` in distributed consensus grammar after P3a.
