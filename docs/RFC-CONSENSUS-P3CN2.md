...
**Status:** APPROVED  
**Stage:** P3c-N2 RFC approval gate  
...
**Repository mutation:** DOCUMENTATION RFC + APPROVAL ONLY  
**Approval status:** APPROVED FOR IMPLEMENTATION  
**Implementation status:** MAY START ONLY AFTER THIS APPROVAL PATCH MERGES  
**Evidence status:** NOT STARTED  
**Capability status after approval:** APPROVED_FOR_IMPLEMENTATION  
...
This document is both the approved P3c-N2 RFC and the approval record for a later implementation PR.

Approved state after this patch merges:

```text
RFC status: APPROVED
Approval status: APPROVED FOR IMPLEMENTATION
Implementation PR allowed: YES, after this approval patch merges
Implementation base: the merge commit of this approval patch
Evidence status: NOT STARTED
P3c-N2 closure status: NOT CLOSED
```

This patch itself is documentation-only. It changes only `docs/RFC-CONSENSUS-P3CN2.md` and does not change runtime code, tests, evidence, matrix, parser, AST, lexer, network, daemon, scheduler, timer, dependency, or config files.

A later implementation PR must branch from the merge commit of this approval patch and must obey the contract, allowlist, non-claims, and stop-gates below.
...
P3c-N2 is the approved design contract for fresh `DistributedConsensusStmt` mailbox-backed vote request delivery and initial collection.

P3c-N2 exists because the runtime can create deterministic consensus decisions and pending consensus tickets, and P3c-N1 can consume mailbox-delivered vote responses for existing pending tickets, but the runtime still does not create or deliver mailbox vote requests from a fresh `DistributedConsensusStmt`.

P3c-N2 adds the request-delivery layer between initial deferred consensus/ticket creation and mailbox-backed response collection.

P3c-N2 does not replace `ConsensusEngine`, does not change consensus mathematics, and does not claim production distributed consensus protocol behavior.

---

## 3. Program Constraints

This RFC follows these program rules:
...
4. Evidence closure must not be mixed into implementation unless approval explicitly changes that rule.
5. Network packet construction must not be represented as network delivery.
6. Test-only wiring must not be represented as production readiness.
7. Replay must remain deterministic.
8. Historical events must not be rewritten.
9. Public signaling must match actual capability maturity.

---

## 4. Current Code Facts

The project already has `DistributedConsensusStmt` with existing fields:
...
The current fresh `DistributedConsensusStmt` path evaluates statement inputs, builds a `ConsensusRequest`, calls `ConsensusEngine.decide(...)`, appends `distributed_consensus_decided`, appends `distributed_consensus_ticket_created` when the result is deferred with `reason = pending_missing_votes`, projects a pending consensus ticket, and binds the decision result to the statement binding.
...
P3c-N2 does not send vote requests before the initial pending ticket exists and does not perform final resolution before mailbox-backed response collection provides the missing votes.
...
The intended classification style is `SUPPORTED_WITH_CRASH_BOUNDARY`; the exact function name is implementation detail.

### 7.2 ConsensusEngine boundary
...
`distributed_consensus_vote_requested` is the domain source of truth. `message_sent` is transport evidence. Replay validates both where both are recorded.
...
Conflicting duplicate request event fails closed. Fresh P3c-N2 response duplicate behavior follows P3c-N1 duplicate semantics after request identity is validated.
...
At interpreter boundary, `ConsensusVoteRequestError` converts to `RuntimeError`. During replay, request mismatch converts to `ConsensusReplayIntegrityError`.
...
### 23.1 RFC/approval-stage files

This combined RFC + approval patch may change only:

```text
docs/RFC-CONSENSUS-P3CN2.md
```

### 23.2 Future implementation allowlist

A future implementation PR may change only the following files unless a later approval correction explicitly narrows this list further:
...
### 23.3 Post-merge evidence allowlist
...
Tests must not use skips or expected failures for P3c-N2 acceptance coverage and must not redefine architecture.
...
This document satisfies the approval requirement for P3c-N2 by recording RFC path, approved document content in this commit, implementation base rule, implementation file allowlist, forbidden files, durable classification decision, engine helper decision, actor delivery local-only decision, participant mailbox binding decision, request event schema, mailbox message schema, request id model, request projection schema, runtime event order, replay contract, failure taxonomy, lifecycle terminal interaction, P3c-N1 compatibility rule, required tests, explicit non-claims, platform baseline requirements, and stop-gates.

Implementation must not begin until this approval patch is merged.

---

## 29. Commit / Push / PR Contract for Later Stages

### 29.1 RFC + approval PR
...
### 29.2 Implementation PR

Allowed mutation only within approved implementation allowlist.

No evidence/matrix update unless a later approval correction explicitly changes the rule. No unrelated docs cleanup. No parser/AST/lexer. No actor runtime change. No network/daemon/timer/scheduler. No dependency/config change.
...
Implementation report must include `TARGET_SHA`, branch, changed files, runtime files changed, test files changed, forbidden files touched yes/no, product statement, canonical owner confirmation, canonical runtime path confirmation, observable result confirmation, durable event confirmation, replay behavior confirmation, idempotency confirmation, compatibility confirmation, failure taxonomy confirmation, test commands, test results, `new_failures`, known platform failures, stop-gates encountered, and scope deviations.
...
This RFC and approval record do not claim production distributed consensus protocol behavior, network/daemon vote delivery, remote participant vote delivery, durable timer/scheduler behavior, persistent durable inbox behavior, parser/AST/lexer expansion, live LLM vote production, public ticket API, automatic rebinding of original deferred consensus variables, overall P3 production status, overall P3c closure, or P3c-N2 evidence closure.
...
P3c-N2 may be marked CLOSED only after production contract is implemented, canonical owner is proven, canonical runtime path is proven, observable result is proven, failure paths are proven, durable/replay semantics are proven, backward compatibility is proven, new regressions are zero, platform baseline is respected, documentation is synchronized, scope is respected, and evidence is recorded.
...
P3c-N2 is the unified approved design contract for fresh `DistributedConsensusStmt` mailbox-backed vote request delivery and initial collection.
...
Implementation may start only after this approval patch merges.
