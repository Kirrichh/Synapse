...
from synapse.experiments.gold.replay import BehaviorReplayResult, validate_replay_result
from synapse.experiments.gold.retrieval import RetrievalCausalRecord, validate_retrieval_causal_record
from synapse.experiments.gold.stage10.context import (
    AdmittedKnowledgeItem,
    ContextSizeBudget,
    ExcludedKnowledgeRef,
)
from synapse.experiments.gold.stage10.intent import IntentCandidate, validate_intent_candidate
...
class PreparedAttemptInputs:
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.knowledge_items) is not tuple or any(
            ...
            raise _fail(GoldRunFailureCode.TYPE_MISMATCH, "knowledge items must be an exact tuple")
        if type(self.excluded_refs) is not tuple or any(
            type(item) is not ExcludedKnowledgeRef for item in self.excluded_refs
        ):
            raise _fail(GoldRunFailureCode.TYPE_MISMATCH, "excluded refs must be an exact tuple")
